...
function Item(props){
    var style = {paddingLeft: ((props.level ? 25 : 10) + props.level * 10) + "px"};
        ...
        if(props.data.url){
            return (
                <li className={"level"+props.level}>
                    <a href={props.data.url} style={style}>{props.data.label}</a>
                </li>
            )
        } else {
            return (
    ...
    function render(){
        return (
            <div className="sidemenu">
                <ul>
                    {data.map((item) => (<Item key={Math.random()} data={item} level={0}/>))}
                    {data.map((item) => (<Item key={Math.random()} data={item} level={0}/>))}
                    {data.map((item) => (<Item key={Math.random()} data={item} level={0}/>))}
                </ul>
            </div>
